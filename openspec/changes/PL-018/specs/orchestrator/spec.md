...
**Reason:** Superseded by the queue dispatch (ADR-001). The pipeline no longer
runs all four stages inline in one invocation; each stage runs in its own Worker
invocation triggered by its own queue message.

**Migration:** Callers that invoked `runOnce(arxivId)` now call
`enqueueDiscovery(trigger)`, which fans the run out over the queue. The
end-to-end "arXiv id → published Post" path is preserved across the
discover/digest/style/publish messages.

### Requirement: Dev-only server trigger

**Reason:** The dev-only `POST /dev/run-once` route invoked the removed inline
`runOnce`. The pipeline is now entered through `enqueueDiscovery` (wired to the
auth-gated `triggerRun` procedure in PL-020 and to Cron in PL-024).

**Migration:** Remove the `POST /dev/run-once` route from `apps/server`. The
server gains a `queue()` consumer instead (see ADDED requirements). The `fetch`
handler, auth, and `AppRouter` type are unchanged.
