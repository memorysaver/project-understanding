...
- [ ] 1.1 Add a pipeline Queue binding (producer + consumer) to the server's
...
- [ ] 2.1 In `packages/orchestrator/src/index.ts`, implement
...
- [ ] 3.1 Implement the `discover` handler: enumerate new Papers (persisted via
...
- [ ] 3.2 Implement the `digest` handler: load the Paper, call `digestor.run`
  (one LLM call), confirm the `Digest` intermediate is persisted and the Paper
  advanced to `digested`, then enqueue `style`.
- [ ] 3.3 Implement the `style` handler: load the `Digest` intermediate + active
...
- [ ] 3.4 Implement the `publish` handler: load the styled body + Paper, call
...
- [ ] 4.1 Add the resume-from-stage guard: a stage re-advances `Paper.status`
...
- [ ] 4.2 On a stage that exhausts `max_retries`, set `Paper.status = failed`,
  record the failure on the `Run`, and enqueue no further stage for that paper.

## 5. Server queue consumer

- [ ] 5.1 In `apps/server/src/index.ts`, add the `queue()` consumer that reads
...
- [ ] 6.5 Integration test (batch): a discovery enqueue run, driven through the
  wired queue with a mocked llm + fixture fetchers, produces published Posts for
  a batch of papers.
- [ ] 6.6 Repo-wide `bun run check-types` passes; the server edit keeps
