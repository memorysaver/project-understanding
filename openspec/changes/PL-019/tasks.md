...
- [x] 1.1 Add a batch arXiv query in `packages/crawler/src/arxiv.ts` — query the
...
- [x] 1.2 Add a multi-entry Atom parser (alongside `parseArxivAtom`) that maps every
...
- [x] 2.1 Add `discover({ db, fetcher?, maxResults?, query? }): Promise<Paper[]>` to
...
- [x] 2.2 Persist-only-new + return new papers: compute which arxiv_ids were not in
...
- [x] 2.3 Honor arXiv etiquette: if the run issues more than one request (paging),
  space requests by `ARXIV_MIN_INTERVAL_MS`; a single-page run is one request.

## 3. Verification

- [x] 3.1 Unit test (dedup across runs): running `discover` twice over the same
...
- [x] 3.2 Integration test (discovery persists only new papers): seed some papers
...
- [x] 3.3 Unit test (User-Agent): a `discover` request carries the custom `USER_AGENT`
  header (assert on the recorded fixture-fetcher call).
- [x] 3.4 `bun run check-types` passes repo-wide.
