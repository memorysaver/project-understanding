...
- [ ] 1.1 Add a `source_kind` column to the `digests` table in
...
- [ ] 1.2 Regenerate the drizzle migration into `packages/db/src/migrations/` via
...
- [ ] 2.1 In `packages/digestor/src/index.ts` `run`, set
...
- [ ] 3.1 In `packages/orchestrator/src/index.ts` `handleDigest`, after the Digest
  exists, read its `source_kind`. For `full_text`, enqueue `style` as today.
- [ ] 3.2 For `source_kind = abstract` and within the deferral budget, **defer**: do
  ...
  does not reset it.
- [ ] 3.3 For `source_kind = abstract` with the deferral budget exhausted, **bounded
...
- [ ] 4.1 Relax the at/past-`digested` resume guard in `handleDigest` only for an
...
- [ ] 5.1 Unit test (digestor): a digest produced from full text records
  `source_kind = full_text`; a digest produced from the abstract fallback records
  `source_kind = abstract`.
- [ ] 5.2 Unit test (orchestrator defers): an abstract-only paper does not enqueue
  `style`, is re-enqueued for a later `digest` retry, and a redelivery does not reset
  the backoff.
- [ ] 5.3 Unit test (orchestrator flags at budget): an abstract-only paper whose
  deferral budget is exhausted is published with the explicit lower-confidence flag,
  not deferred again.
- [ ] 5.4 Unit test (re-digest): a paper whose current Digest is `abstract`, when
  retried with full text now available, is re-digested to `source_kind = full_text`
  before proceeding to publish.
- [ ] 5.5 Integration test: an abstract-only paper driven through the pipeline is not
...
- [ ] 5.6 `bun run check-types` passes repo-wide.
